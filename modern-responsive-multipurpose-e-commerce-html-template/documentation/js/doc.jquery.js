...
(function($){
	...
	*/
	...
	jQuery.fn[pluginNS]=function(method){ /* usage: $(selector).mCustomScrollbar(); */
		if(methods[method]){
	...
	jQuery[pluginNS]=function(method){ /* usage: $.mCustomScrollbar(); */
		if(methods[method]){
			return methods[method].apply(this,Array.prototype.slice.call(arguments,1));
		}else if(typeof method==="object" || !method){
			return methods.init.apply(this,arguments);
...
(function($) {
	...
	//Add OnepageNav / mt-sidebar
	function sideNav() {
		if(jQuery('.menu-box .sticky-menu').length){
			jQuery('.menu-box .sticky-menu ul').onePageNav();
		}
	}
	
	//Add Scroll Bar To mt-sidebar
	if(jQuery('#mt-sidebar .menu-box').length){
		jQuery("#mt-sidebar .menu-box").mCustomScrollbar({
			axis:"y",
			autoExpandScrollbar:false
		});
	}
